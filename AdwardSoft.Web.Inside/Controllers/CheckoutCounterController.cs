...
namespace AdwardSoft.Web.Inside.Controllers
{
    public class CheckoutCounterController : Controller
    {
        ...
        public CheckoutCounterController(IUserSession userSession, IAPIFactory apiFactory, IHttpContextAccessor httpContextAccessor)
        {
        ...
        {
            var model = new CheckoutCounterViewModel { Id = id };
        ...
        [HttpGet]
        public async Task<IEnumerable<CheckoutCounterDatatableViewModel>> Read()
        {
            var response = await _apiFactory.GetAsync<List<CheckoutCounterDatatableViewModel>>(
        ...
        [HttpPost]
        public async Task<ResponseContainer> Create(CheckoutCounterDatatableViewModel vm)
        {
            vm.BranchId = _BranchId;

            var result = await _apiFactory.PostAsync<CheckoutCounterDatatableViewModel, int>(
                data: vm,
        ...
        [HttpPut]
        public async Task<ResponseContainer> Update(CheckoutCounterDatatableViewModel vm)
        {
            vm.BranchId = _BranchId;

            var result = await _apiFactory.PutAsync<CheckoutCounterDatatableViewModel, int>(
                data: vm,
        ...
        private async Task<CheckoutCounterViewModel> _ReadByIdAsync(int id)
        {
            var response = await _apiFactory.GetAsync<CheckoutCounterViewModel>(
